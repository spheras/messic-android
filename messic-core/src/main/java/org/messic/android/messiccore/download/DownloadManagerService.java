...
public class DownloadManagerService
    ...
    public void addDownload(MDMAlbum album, Context context, DownloadListener olistener) {
        ...
        for (MDMSong mdmSong : songs) {
            addDownload(mdmSong, context, olistener);
        }
    }

    public void addDownload(MDMSong song, Context context, final DownloadListener olistener) {
        UtilDownloadService.notification.setService(this);
        UtilDownloadService.notification.downloadAdded(song);
        ...
        try {
            DownloadQueue.DownloadQueueListener listener = new DownloadQueue.DownloadQueueListener() {
                public void received(MDMSong song, File fdownloaded) {
                    UtilDownloadService.notification.downloadFinished(song, fdownloaded);
                ...
                public void started(MDMSong song) {
                    UtilDownloadService.notification.downloadStarted(song);
                }
            };
