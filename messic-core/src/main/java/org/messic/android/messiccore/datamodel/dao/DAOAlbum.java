...
public class DAOAlbum
    ...
    public DAOAlbum() {
        super(MDMAlbum.TABLE_NAME, MDMAlbum.getColumns());
    }
    ...
    public List<MDMAlbum> getAllByAlbum() {
        ...
        while (!cursor.isAfterLast()) {
            MDMAlbum msi = new MDMAlbum(cursor, true, false);
            result.add(msi);
            cursor.moveToNext();
        }
    ...
    public MDMAlbum getByAlbumLSid(int albumLSid, boolean loadSongs) {
        ...
        if (c.moveToFirst()) {
            result = new MDMAlbum(c, loadSongs, false);
            c.moveToNext();
        }
        c.close();
        close();
    ...
    public List<MDMAlbum> getAllByAuthorLSid(int authorLSid, boolean loadSongs) {
        ...
        if (c.moveToFirst()) {
            MDMAlbum album = new MDMAlbum(c, loadSongs, false);
            result.add(album);
            c.moveToNext();
        }
        c.close();
    ...
    public MDMAlbum save(MDMAlbum album, boolean saveSongs) {
        ...
        albumCV.put(MDMAlbum.COLUMN_VOLUMES, album.getVolumes());
        ...
        if (album.getAuthor() == null) {
            ...
            if (album.getAuthor().getLsid() > 0) {
                ...
                if (cAuthor.getCount() > 0) {
                    ...
                    // it doesn't exists at the database, we need to create it previously
                    MDMAuthor author = daoauthor.save(album.getAuthor(), false);
                    albumCV.put(MDMAlbum.COLUMN_FK_AUTHOR, author.getLsid());
                }
                cAuthor.close();
                daoauthor.close();
        ...
        c.moveToFirst();
        MDMAlbum msi = new MDMAlbum(c, true, false);
        c.close();
        close();
        return msi;
    }
