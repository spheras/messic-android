...
public class PlayQueueSongAdapter extends RecyclerView.Adapter<PlayQueueSongViewHolder> {

    @Inject
    Configuration config;
    private List<MDMSong> songs;
    private PlayQueueSongViewHolder.IViewHolderClicks listener;
    // <0 to indicate that not current song want to be highlighted
    private int currentSong = -1;

    public PlayQueueSongAdapter() {
    ...
    public void setListener(PlayQueueSongViewHolder.IViewHolderClicks listener) {
    ...
    @Override
    public void onBindViewHolder(PlayQueueSongViewHolder holder, int position) {
        ...
        if (position == getCurrentSong()) {
            holder.rlBase.setBackgroundResource(R.color.green);
        } else if (position % 2 == 0) {
            holder.rlBase.setBackgroundColor(Color.WHITE);
        } else {
            holder.rlBase.setBackgroundColor(Color.parseColor("#F8F8F8"));
        }
